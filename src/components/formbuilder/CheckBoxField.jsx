...
function CheckBoxField({ index, fieldData, setLabel, handleFieldChange, fieldTypesList, handleRemoveField, setIsRequired, addMoreOption, handleOptionNameChange, handleDeleteOption },) {
            ...
            <div className='bg-gray-200 rounded-md my-4 p-2 flex flex-wrap'>
                <div className='p-1 '>
                    {fieldData.options.map((opt, id) => {
                        return <div key={id} className='flex justify-start items-center'><div className='w-5 h-5 m-2 border-2 bg-white'></div> <input onChange={(e) => handleOptionNameChange(index, id, e.target.value)} type="text" value={opt} className='bg-inherit ' /> <div ><RxCross2 onClick={() => handleDeleteOption(index, id)} /></div></div>

                    })}
                    <div className='flex justify-start items-center p-1 ml-4 text-gray-600 hover:text-blue-300 cursor-pointer' onClick={() => addMoreOption(index)}><div className='w-5 h-5 m-2 border-2 bg-white'></div>add option</div>
                    {/* <div>add option</div> */}
                </div>
            </div>
