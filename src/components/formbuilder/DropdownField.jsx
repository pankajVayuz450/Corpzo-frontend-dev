...
function DropdownField({ index, fieldData, setLabel, handleFieldChange, fieldTypesList, handleRemoveField, setIsRequired, addMoreOption, handleOptionNameChange, handleDeleteOption },) {
                    ...
                    {fieldData.options.map((opt, id) => {
                        return (
                            <div key={id} className='flex justify-start items-center'>
                                ...
                                <div className='w-5 h-5 m-2 '>{id + 1}.</div>
                                <input onChange={(e) => handleOptionNameChange(index, id, e.target.value)} type="text" value={opt} className='bg-inherit ' />
                                <div ><RxCross2 onClick={() => handleDeleteOption(index, id)} /></div>
                            </div>)
